...
async function remote(url) {
  const response = await fetch(url, { headers: { 'User-Agent': 'London-Visitor-Compass/1.0 (+local development server)', Accept: 'application/json, application/rss+xml, application/xml, text/html;q=0.9' }, signal: AbortSignal.timeout(12000) });
...
async function liveUpdates() {
  ...
  return { updatedAt: time(new Date()), updates };
}
...
createServer(async (request, response) => {
  ...
  if (url.pathname === '/api/updates') {
    try {
      const payload = await liveUpdates();
      response.writeHead(200, { 'Content-Type': MIME['.json'], 'Cache-Control': 'no-store' });
      response.end(JSON.stringify(payload));
    } catch (error) {
      response.writeHead(502, { 'Content-Type': MIME['.json'], 'Cache-Control': 'no-store' });
      response.end(JSON.stringify({ error: 'Live updates are temporarily unavailable.' }));
    }
    return;
  }
...
}).listen(port, () => console.log(`London Visitor Compass is running at http://localhost:${port}`));
